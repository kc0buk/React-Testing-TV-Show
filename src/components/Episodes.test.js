...
test('Render missions when data fetch finishes', () => {
    const { rerender } = render(<Episodes episodes={[]} />)

    let episodeDivs = screen.queryAllByTestId(/episodeDiv/i)
    expect(episodeDivs).toHaveLength(0)

    rerender(<Episodes episodes={seasons} />)
    // screen.debug()

    episodeDivs = screen.queryAllByTestId(/episodeDiv/i)
